    ...
    reader.onload = async () => {
      try {
        ...
        setFormData(prev => ({
          ...prev,
          ...extracted,
          owners: extracted.owners || prev.owners,
          tenants: extracted.tenants || prev.tenants,
        ...
        <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed rounded-[3rem] p-20 border-slate-800 bg-slate-900/40 animate-scan">
          {isProcessing ? (
             <div className="text-center space-y-6">
               ...
               <h3 className="text-3xl font-black text-white italic tracking-tighter uppercase">Lia sta analizzando...</h3>
        ...
        <div className="flex-1 flex flex-col min-h-0 bg-slate-900 border border-white/5 rounded-[2.5rem] p-8 backdrop-blur-xl relative hud-border overflow-hidden">
            <div className="flex items-center justify-between mb-8 pb-6 border-b border-white/5">
                <h2 className="text-2xl font-black text-white italic uppercase tracking-tighter flex items-center gap-3">
                    <Clock className="w-7 h-7 text-primary-500" /> Verifica Dati Estrazione <span className="text-slate-700">/</span> <span className="text-primary-500">{formData.propertyAddress || 'Revisione'}</span>
                </h2>
            ...
            <div className="flex gap-3 mb-8 overflow-x-auto no-scrollbar">
                {[
                  ...
                  { id: 'economico', label: 'Canoni e Fisco', icon: Euro },
                ...
                {activeTab === 'anagrafica' && (
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                      <div className="space-y-6 p-8 bg-slate-950/40 rounded-[2rem] border border-white/5">
                          <h4 className="text-xs font-black text-primary-500 uppercase tracking-[0.3em] mb-4">Proprietà (Locatori)</h4>
                      ...
                      <div className="space-y-6 p-8 bg-slate-950/40 rounded-[2rem] border border-white/5">
                          <h4 className="text-xs font-black text-indigo-400 uppercase tracking-[0.3em] mb-4">Locatari (Conduttori)</h4>
                ...
                {activeTab === 'economico' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-6 bg-slate-950/40 p-8 rounded-[2rem] border border-white/5">
                          <h4 className="text-xs font-black text-primary-500 uppercase tracking-[0.3em]">Durata</h4>
                      ...
                      <div className="space-y-6 bg-slate-950/40 p-8 rounded-[2rem] border border-white/5">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <InputField label="Mesi Preavviso" type="number" value={formData.noticeMonthsOwner} onChange={(v: number) => setFormData(p => ({...p, noticeMonthsOwner: v, noticeMonthsTenant: v}))} icon={Info} />
                          </div>
                          <div className="flex gap-4">
