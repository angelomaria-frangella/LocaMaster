  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-1000 pb-20">
      ...
      <div className="relative p-10 lg:p-16 rounded-[3rem] lg:rounded-[5rem] bg-slate-950 border border-white/10 shadow-3xl overflow-hidden group">
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-primary-500 to-transparent animate-scan opacity-40"></div>
          <div className="absolute -bottom-48 -right-48 w-[40rem] h-[40rem] bg-primary-600/10 blur-[150px] rounded-full"></div>
              ...
              <div className="space-y-10">
                  <div className="flex flex-wrap gap-4">
                      ...
                      <div className="flex items-center gap-2.5 px-5 py-2 rounded-full border border-primary-500/30 bg-primary-500/10 text-[10px] font-black uppercase tracking-widest text-primary-400 shadow-[0_0_15px_rgba(99,102,241,0.2)]">
                          <Activity className="w-3.5 h-3.5 text-primary-500 animate-pulse" /> V1.4.0 - STABLE MASTER
                      </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <p className="text-slate-400 font-bold text-xl max-w-lg border-l-4 border-primary-500/40 pl-8 py-3 leading-relaxed">
                        Benvenuto, Dott. Frangella.<br/>
                        <span className="text-white">Radar attivo su {contracts.length} asset immobiliari.</span>
                    </p>
                  </div>
              </div>

              <div className="flex flex-col gap-5 min-w-[320px] justify-center">
                  <button onClick={onAddContract} className="w-full py-8 bg-primary-600 hover:bg-primary-500 text-white rounded-[2.5rem] font-black text-2xl shadow-[0_25px_60px_rgba(79,70,229,0.5)] flex items-center justify-center gap-5 transition-all active:scale-95 group/btn border border-primary-400/30">
                      <FilePlus className="w-10 h-10 group-hover/btn:rotate-12 transition-transform" /> NUOVA PRATICA
                  </button>
                  <button onClick={() => setIsReportModalOpen(true)} className="w-full py-6 bg-slate-900 hover:bg-slate-800 text-slate-300 rounded-[2rem] font-bold text-lg flex items-center justify-center gap-4 transition-all border border-white/10 hover:border-primary-500/50 shadow-2xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 p-12 bg-slate-900/50 border border-white/5 rounded-[4rem] relative overflow-hidden shadow-2xl backdrop-blur-xl">
              <div className="absolute top-0 right-0 w-96 h-96 bg-primary-600/5 blur-[120px] rounded-full"></div>
              <div className="relative z-10 space-y-10">
                  <h3 className="font-black text-white text-3xl uppercase tracking-tighter flex items-center gap-6 italic">
                  ...
                  <div className="grid grid-cols-1 gap-6">
                      {isInsightsLoading ? (
                        ...
                        <div className="py-20 text-center space-y-6 opacity-40">
                            ...
                            <p className="text-slate-500 font-black italic uppercase tracking-[0.4em] text-sm">Sincronizzazione parametri IA in corso...</p>
                        </div>
                      )}
                  </div>
              </div>
          ...
          {/* PERFORMANCE MONITOR */}
          <div className="p-12 bg-slate-900/50 border border-white/10 rounded-[4rem] flex flex-col justify-between text-center relative overflow-hidden shadow-2xl backdrop-blur-xl group">
               <div className="absolute inset-0 bg-gradient-to-b from-primary-500/10 to-transparent opacity-40"></div>
               <span className="text-[12px] font-black text-slate-500 tracking-[0.6em] uppercase relative z-10 group-hover:text-primary-400 transition-colors">PORTFOLIO HEALTH</span>
               <div className="relative z-10 py-12">
                   <div className="text-emerald-400 font-black text-[10rem] tracking-tighter leading-none drop-shadow-[0_0_40px_rgba(16,185,129,0.4)]">98<span className="text-4xl ml-1">.4%</span></div>
               ...
               <div className="space-y-8 relative z-10">
                   <div className="h-6 bg-slate-950 rounded-full overflow-hidden border border-white/10 p-1.5 shadow-2xl">
                       <div className="h-full bg-gradient-to-r from-primary-600 via-indigo-400 to-emerald-400 rounded-full w-[98.4%] shadow-[0_0_30px_rgba(99,102,241,0.6)]"></div>
                   </div>
                   <p className="text-[11px] font-black text-slate-500 uppercase tracking-[0.4em]">Operational Readiness Status</p>
               </div>
          </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          {/* VOLUME CHART */}
          <div className="xl:col-span-2 p-12 bg-slate-900/50 border border-white/5 rounded-[4rem] shadow-3xl overflow-hidden relative group backdrop-blur-xl">
              ...
              <div className="h-[28rem] w-full relative z-10">
                  <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={workloadData}>
                          ...
                          />
                          <Area type="monotone" dataKey="value" stroke="#6366f1" strokeWidth={10} fillOpacity={1} fill="url(#colorValue)" animationDuration={2500} />
                      </AreaChart>
                  </ResponsiveContainer>
              </div>
          </div>
          ...
          <div className="bg-slate-900 border border-white/5 rounded-[4rem] p-12 flex flex-col h-full shadow-3xl relative overflow-hidden backdrop-blur-xl">
              <div className="absolute top-0 right-0 w-full h-1 bg-gradient-to-r from-transparent via-rose-500/30 to-transparent"></div>
              ...
              <div className="space-y-6 flex-1 overflow-y-auto pr-2 scrollbar-hide">
                  {deadlines.length > 0 ? (
                      deadlines.slice(0, 8).map(d => (
                          <div key={d.id} className="p-7 bg-white/5 border border-white/5 rounded-[2.5rem] flex items-center justify-between hover:bg-white/10 transition-all duration-500 group cursor-default hover:border-primary-500/30">
                              <div className="flex-1 min-w-0 mr-6">
                                  <p className="text-[11px] font-black text-slate-500 uppercase tracking-[0.4em] mb-2">{new Date(d.date).toLocaleDateString('it-IT', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                                  ...
                                  <p className="text-[11px] text-slate-500 truncate font-bold uppercase tracking-widest mt-1 opacity-60">{d.contractAddress}</p>
                              </div>
                              <div className={`px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-tighter shadow-2xl border ${d.urgency === 'critical' ? 'bg-rose-600 text-white animate-pulse border-rose-400' : 'bg-primary-500/10 text-primary-400 border-primary-500/20'}`}>
                                  {d.urgency}
                              </div>
                          </div>
                      ))
...
const KPICard = ({ title, value, icon: Icon, color, alert }: any) => (
    <div className={`p-12 bg-slate-900 border rounded-[4rem] transition-all duration-700 hover:scale-[1.05] hover:shadow-[0_40px_80px_rgba(0,0,0,0.5)] group relative overflow-hidden ${alert ? 'border-rose-500/50 bg-rose-950/10' : 'border-white/5 hover:border-primary-500/40'}`}>
        ...
        <div className="flex justify-between items-start mb-10 relative z-10">
            ...
            <div className={`p-5 rounded-3xl transition-all duration-700 ${alert ? 'bg-rose-500 text-white shadow-[0_0_30px_rgba(244,63,94,0.5)]' : `bg-${color}-500/10 text-${color}-400 group-hover:scale-125 shadow-inner border border-${color}-500/20`}`}>
