...
import { 
  ...
  ArrowUpRight, Landmark, CreditCard, Clock, MapPin
} from 'lucide-react';
        ...
        <div className="lg:col-span-2 p-10 hud-border rounded-3xl flex flex-col md:flex-row justify-between items-center gap-8 overflow-hidden">
           <div className="relative">
              ...
              <div className="flex items-center gap-4 mt-4">
                 ...
                 <span className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.3em]">PROT. GESTIONALE V5.0</span>
              </div>
           </div>
            ...
            <div className="space-y-6">
              {isInsightsLoading ? (
                ...
                aiInsights.map((insight, i) => (
                  <div key={i} className={`p-8 border rounded-[2rem] flex items-start gap-8 transition-all ${insight.text.includes('CARENZA') || insight.text.includes('RISCHIO') ? 'hazard-alert' : 'bg-slate-950/40 border-white/5 hover:border-primary-500/40 hover:bg-slate-900/60'}`}>
                    <div className={`p-5 rounded-2xl flex-shrink-0 ${insight.text.includes('CARENZA') || insight.text.includes('RISCHIO') ? 'bg-rose-500 text-white shadow-[0_0_20px_#f43f5e]' : 'bg-primary-600/10 text-primary-500 border border-primary-500/30'}`}>
                      {insight.text.includes('CARENZA') || insight.text.includes('RISCHIO') ? <AlertCircle className="w-8 h-8" /> : <Target className="w-8 h-8" />}
                    </div>
                    <div className="flex-1">
                      <p className={`text-[11px] font-black uppercase tracking-[0.3em] mb-3 ${insight.text.includes('CARENZA') || insight.text.includes('RISCHIO') ? 'text-rose-400' : 'text-primary-500'}`}>{insight.category}</p>
            ...
            {deadlines.length > 0 ? deadlines.slice(0, 10).map((d, i) => (
              <div key={i} className="p-6 bg-slate-950/50 border border-white/5 rounded-2xl hover:border-primary-500/40 transition-all group cursor-pointer relative overflow-hidden">
                ...
                <div className="flex items-center gap-2 text-[10px] text-slate-500 font-bold uppercase tracking-tighter">
                    {/* Added missing MapPin icon import */}
